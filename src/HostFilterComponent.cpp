...
#include "resources/Resources.h"
  ...
  #include "formats/Standalone/juce_AudioFilterStreamer.h"
  #include "formats/Standalone/juce_StandaloneFilterWindow.h"
#endif
...
void HostFilterComponent::loadPluginFromFile (const File& file)
{
    ...
    if (graph) graph->loadAndAppendPlugin (file);
}
    ...
    case 0: // CommandCategories::file
        {
            ...
            if (fileID >= 0 && fileID < config->recentPlugins.getNumFiles())
            {
                ...
                if (graph)
                    graph->loadAndAppendPlugin (config->recentPlugins.getFile (fileID));
