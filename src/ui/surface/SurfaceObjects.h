...
public:
    ...
    void setPlugin (uint32 newPlugin)                 { if (plugin != newPlugin) { plugin = newPlugin; removeAllParameters (); } }
    uint32 getPlugin () const                         { return plugin; }

    void setParameter (const int index, const int n)  { parameters.set (index, n); }
    int getParameter (const int index) const          { return parameters [index]; }
    void removeAllParameters ()                       { parameters.clear (); }
